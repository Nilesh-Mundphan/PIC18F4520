...
void lcd_init(int d4,int d5,int d6,int d7,int rs,int en)
{
 ...
 EN=en;
    for (i=0;i<4;i++)
    {
        gpio_pin_mode(lcd_data_pins[i],0);
    }
...
void lcd_nibble(unsigned char nibble)
{
  ...
  {
      if((( nibble >> i) & 1) == 1)
      gpio_pin_write(lcd_data_pins[i], 1);
      else
      gpio_pin_write(lcd_data_pins[i], 0);
  }
}
