...
unsigned int serial_available(void)
{
 ...
 if(rx_Buffer_Head < rx_Buffer_Tail)
   ...
   return (256 - rx_Buffer_Head + rx_Buffer_Tail + 1);
 else
  return 1;
}
char serial_read(void)
...
{
 char rxData=RCREG;
  if((rx_Buffer_Tail == 256 -1 && (rx_Buffer_Head == 0)) || ((rx_Buffer_Tail + 1) == rx_Buffer_Head))
   {
    rx_Buffer_Head++;
   }

   if(rx_Buffer_Head == 256 || rx_Buffer_Head == -1)
    rx_Buffer_Head=0;

   if(rx_Buffer_Tail == 256 - 1)
     rx_Buffer_Tail=0;
   else
     rx_Buffer_Tail++;
